        ...
        <div class="w-full overflow-x-auto">
            <table class="w-full whitespace-no-wrap">
                ...
                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                      @foreach($layananTTEs as $layananTTE)
                    <tr class="text-gray-700 dark:text-gray-400">
                        <td class="px-4 py-3">{{ $layananTTE->nama_lengkap }}</td>
                        <td class="px-4 py-3">{{ $layananTTE->ktp }}</td>
                        <td class="px-4 py-3">{{ $layananTTE->nik }}</td>
                        <td class="px-4 py-3">{{ $layananTTE->nip }}</td>
                        <td class="px-4 py-3">{{ $layananTTE->jabatan }}</td>
                        <td class="px-4 py-3">{{ $layananTTE->no_hp }}</td>
                        <td class="px-4 py-3">{{ $layananTTE->unit_kerja }}</td>
                        <td class="px-4 py-3">{{ $layananTTE->tanggal_permohonan ? \Carbon\Carbon::parse($layananTTE->tanggal_permohonan)->format('d/m/Y') : 'N/A' }}</td>
                        <td class="px-4 py-3">{{ $layananTTE->waktu_permohonan ? \Carbon\Carbon::parse($layananTTE->waktu_permohonan)->format('H:i') : 'N/A' }}</td>
                        <td class="px-4 py-3">{{ $layananTTE->kategori ? $layananTTE->kategori->kategori_layanan : 'N/A' }}</td>
                        <td class="px-4 py-3">{{ $layananTTE->perangkatDaerah ? $layananTTE->perangkatDaerah->perangkat_daerah : 'N/A' }}</td>
                        <td class="px-4 py-3">
                            @php
        ...
        <div class="flex flex-col px-4 py-3 text-sm font-medium text-gray-500 uppercase border-t dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
            <div class="flex items-center justify-between">
                <span class="text-gray-700 dark:text-gray-300">
                    Menampilkan {{ $layananTTEs->firstItem() }} hingga {{ $layananTTEs->lastItem() }} dari {{ $layananTTEs->total() }} hasil
                </span>
                <div class="flex items-center">
                    <!-- Pagination Links -->
                    {{ $layananTTEs->links('pagination::tailwind') }}
                </div>
            </div>
        </div>
    </div>
