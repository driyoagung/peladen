        ...
        <div class="w-full overflow-x-auto">
            <table class="w-full whitespace-no-wrap">
                ...
                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                    @foreach($layananVPNs as $layananVPN)
                    <tr class="text-gray-700 dark:text-gray-400">
                        <td class="px-4 py-3">{{ $layananVPN->nama_pemohon }}</td>
                        <td class="px-4 py-3">{{ $layananVPN->nip_nik }}</td>
                        <td class="px-4 py-3">{{ $layananVPN->lokasi }}</td>
                        <td class="px-4 py-3">{{ $layananVPN->unit_kerja }}</td>
                        <td class="px-4 py-3">{{ $layananVPN->tanggal_permohonan ? \Carbon\Carbon::parse($layananVPN->tanggal_permohonan)->format('d/m/Y') : 'N/A' }}</td>
                        <td class="px-4 py-3">{{ $layananVPN->waktu_permohonan ? \Carbon\Carbon::parse($layananVPN->waktu_permohonan)->format('H:i') : 'N/A' }}</td>
                        <td class="px-4 py-3">{{ $layananVPN->kategori ? $layananVPN->kategori->kategori_layanan : 'N/A' }}</td>
                        <td class="px-4 py-3">{{ $layananVPN->perangkatDaerah ? $layananVPN->perangkatDaerah->perangkat_daerah : 'N/A' }}</td>
                        <td class="px-4 py-3">
                            @php
                                $status = $layananVPN->statusPermohonan ? $layananVPN->statusPermohonan->status : 'N/A';
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center space-x-4 text-sm">
                                <a href="{{ route('admin.layananVPN.edit', $layananVPN->id) }}" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Edit">
                                    <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path>
                                    </svg>
                                </a>
                                <form action="{{ route('admin.layananVPN.destroy', $layananVPN->id) }}" method="POST" class="delete-form">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="flex flex-col px-4 py-3 text-sm font-medium text-gray-500 uppercase border-t dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
            <div class="flex items-center justify-between">
                <span class="text-gray-700 dark:text-gray-300">
                    Menampilkan {{ $layananVPNs->firstItem() }} hingga {{ $layananVPNs->lastItem() }} dari {{ $layananVPNs->total() }} hasil
                </span>
                <div class="flex items-center">
                    <!-- Pagination Links -->
                    {{ $layananVPNs->links('pagination::tailwind') }}
                </div>
            </div>
        </div>
    </div>
